import { createContext, type SessionAuth } from "@korex/api/context";
import {
...
type ServerAuth = SessionAuth & {
  handler: (request: Request) => Promise<Response>;
};

export function createServerApp({ auth }: { auth: ServerAuth }) {
  const app = new Hono();
  ...
  app.get(
    ...
    async (c) => {
      const context = await createContext({ auth, context: c });
      ...
      try {
        ...
        if (!tileInput) {
          return routeHeatmapTileResponse(
            renderEmptyActivityRouteHeatmapTile(),
          );
        }
