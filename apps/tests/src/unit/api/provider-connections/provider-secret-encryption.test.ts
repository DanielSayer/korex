...
describe("provider secret encryption", () => {
  ...
  it("decrypts a known provider secret payload", async () => {
    ...
    const decrypted = await Effect.runPromise(decryptProviderSecret(encrypted));

    expect(decrypted).toBe("intervals-api-key");
  });

  it("uses a random iv for each encryption", async () => {
    const firstEncrypted = await Effect.runPromise(
      encryptProviderSecret("intervals-api-key"),
    );
    const secondEncrypted = await Effect.runPromise(
      encryptProviderSecret("intervals-api-key"),
    );
